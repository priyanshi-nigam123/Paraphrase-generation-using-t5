...
def generate_paraphrase(input_text, model, tokenizer, device, num_return_sequences=4, diversity_penalty=1.5):
...
st.markdown('<p class="sub-header">Generate multiple natural paraphrases for your sentences using a fine-tuned T5 model.</p>', unsafe_allow_html=True)
    ...
    if not input_sentence.strip():
        ...
        with st.spinner("Generating paraphrases..."):
            st.session_state.results = generate_paraphrase(
                input_sentence, model, tokenizer, device,
...
if st.session_state.results:
    st.markdown("### Results")
    st.markdown(f'<div class="original-card">📌 <b>Original:</b> {st.session_state.input_text}</div>', unsafe_allow_html=True)
    cols = st.columns(2)
    for i, para in enumerate(st.session_state.results, 1):
        with cols[(i - 1) % 2]:
            st.markdown(f'<div class="paraphrase-card"><b>#{i}</b><br>{para}</div>', unsafe_allow_html=True)
else:
